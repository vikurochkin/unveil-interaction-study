...
app.innerHTML = `
  ...
  <nav class="view-switcher" aria-label="Project view">
    <button class="view-button is-active" type="button">Overview</button>
...
const selectProject = (project, element) => {
  if (project.slug === 'nto-stratus') {
    history.pushState({}, '', '/nto-stratus');
  }
  element.classList.add('is-selected');
};
const home = renderHome(routeView, { projects: PROJECTS, onSelect: selectProject });
const overlays = createOverlayController(document, { projects: PROJECTS, onSelect: selectProject });
...
const loadingTimer = window.setInterval(() => {
  percent = Math.min(100, percent + Math.ceil((100 - percent) * 0.19));
